...
* Is there a relationship between both abortion access & hostility towards abortion with abortion rates by state?
...
We pulled in state-level data directly from the [Guttmacher Institute Data Center (2014-2017)](https://data.guttmacher.org/states) as a .csv file, read it in and tidied it as follows:
...
We then manually extracted state-level data on hostility around abortion rights provided by the [Guttmacher Institute](https://www.guttmacher.org/sites/default/files/article_files/hostile_and_supportive_state_abortion_laws_1-24-2020.pdf) into Microsoft Excel, exported it as a .csv file and read it in:
...
We then merged these two datasets into a single dataset, and saved it into our data folder for ease of use:
...
# read in project data and tidy
merge_data_map = read_csv("data/merge_data.csv") %>% 
